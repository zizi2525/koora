...
export default function Home({ matches }) {
  return (
    ...
    <main className="grid grid-cols-6 pb-16 pt-16 dark:bg-zinc-950 ">
      <div></div>
      ...
      <h1 className="text-2xl font-bold text-gray-700 dark:text-gray-300 p-2 m-2">مباريات اليوم</h1>
        ...
        <div className="flex flex-row justify-between items-center">
          ...
          <Link href="/" className="text-md font-bold text-gray-700 dark:text-gray-300 bg-red-500 px-4 py-2 animate-pulse">
            
          مباريات اليوم
          </Link>
         
          <Link href="/yesterday-matches" className="text-md font-bold text-gray-700 dark:text-gray-300  bg-gray-300 px-4 py-2">
          مباريات الأمس
          </Link>
       ...
       <div  key={`match${index}`}  className="relative container my-2 py-2 cursor-pointer group group-hover:text-yellow-500 ">
        {match.match_url !=="#"? (
        <Link className='absolute top-0 left-0 w-full h-full bg-black bg-opacity-30 z-10 items-center justify-center hidden group-hover:flex' href={`/live/${match.match_url}`} alt={`${match.team1} VS ${match.team2}`} title={`${match.team1} VS ${match.team2}`}>
        <FaPlayCircle className='text-teal-500 text-6xl group-hover animate-pulse transition-all decoration-neutral-800 duration-500' />
        </Link>
        ) : (
          <div className='absolute top-0 left-0 w-full h-full bg-black bg-opacity-30 z-10 items-center justify-center hidden group-hover:flex'>
          <FaPlayCircle className='text-teal-500 text-6xl group-hover animate-pulse transition-all decoration-neutral-800 duration-500' />
          </div>
        )}
        ...
        <span className="truncate">{match.league}</span>
         ...
         <div className="match-content flex">
           <div className="column flex items-center justify-center p-4 w-1/3">
             <div className="team team--home flex flex-col items-center justify-center">
